...
public class move_p3 : MonoBehaviour
{
    Rigidbody rbPlayer;
    ...
    void FixedUpdate()
    {
        ...
        vel.z = -Input.GetAxisRaw("V2") * Time.deltaTime * velocidad;
        if (vel.z != 0.0f)
        {
            if (vel.z > 0.0f)
            {
                transform.Rotate(0, -Input.GetAxisRaw("H2") * velocidad * Time.deltaTime, 0);
            }
            else
            {
                transform.Rotate(0, Input.GetAxisRaw("H2") * velocidad * Time.deltaTime, 0);
        ...
        {
            transform.Rotate(0, -Input.GetAxis("H2") * velocidad / 2 * Time.deltaTime, 0);
